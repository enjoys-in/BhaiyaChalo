package event

import (
	"context"
	"log/slog"

	"github.com/enjoys-in/BhaiyaChalo/services/driver-ops/payout-service/internal/model"
	"github.com/enjoys-in/BhaiyaChalo/services/driver-ops/payout-service/internal/ports"
)

type kafkaPublisher struct {
	logger *slog.Logger
}

func NewKafkaPublisher(logger *slog.Logger) ports.EventPublisher {
	return &kafkaPublisher{logger: logger}
}

func (p *kafkaPublisher) PublishPayoutInitiated(ctx context.Context, entity *model.Payout) error {
	// TODO: publish to Kafka
	return nil
}

func (p *kafkaPublisher) PublishPayoutCompleted(ctx context.Context, entity *model.Payout) error {
	// TODO: publish to Kafka
	return nil
}
