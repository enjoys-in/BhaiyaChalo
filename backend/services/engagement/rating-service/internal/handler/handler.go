package handler

import (
	"net/http"

	"github.com/enjoys-in/BhaiyaChalo/services/engagement/rating-service/internal/dto"
	"github.com/enjoys-in/BhaiyaChalo/services/engagement/rating-service/internal/ports"
)

type RatingHandler struct {
	svc ports.RatingService
}

func NewRatingHandler(svc ports.RatingService) *RatingHandler {
	return &RatingHandler{svc: svc}
}

func (h *RatingHandler) Create(w http.ResponseWriter, r *http.Request) {
	var req dto.CreateRatingRequest
	if err := decodeJSON(r, &req); err != nil {
		errorJSON(w, http.StatusBadRequest, "invalid request body")
		return
	}

	resp, err := h.svc.Create(r.Context(), req)
	if err != nil {
		errorJSON(w, http.StatusInternalServerError, err.Error())
		return
	}
	successJSON(w, http.StatusCreated, "rating created successfully", resp)
}

func (h *RatingHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		errorJSON(w, http.StatusBadRequest, "id is required")
		return
	}

	resp, err := h.svc.GetByID(r.Context(), id)
	if err != nil {
		errorJSON(w, http.StatusInternalServerError, err.Error())
		return
	}
	successJSON(w, http.StatusOK, "rating fetched successfully", resp)
}

func (h *RatingHandler) Update(w http.ResponseWriter, r *http.Request) {
	var req dto.UpdateRatingRequest
	if err := decodeJSON(r, &req); err != nil {
		errorJSON(w, http.StatusBadRequest, "invalid request body")
		return
	}

	resp, err := h.svc.Update(r.Context(), req)
	if err != nil {
		errorJSON(w, http.StatusInternalServerError, err.Error())
		return
	}
	successJSON(w, http.StatusOK, "rating updated successfully", resp)
}

func (h *RatingHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		errorJSON(w, http.StatusBadRequest, "id is required")
		return
	}

	if err := h.svc.Delete(r.Context(), id); err != nil {
		errorJSON(w, http.StatusInternalServerError, err.Error())
		return
	}
	successJSON(w, http.StatusOK, "rating deleted successfully", nil)
}
