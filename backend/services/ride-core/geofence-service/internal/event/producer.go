...
import (
	...
	"github.com/enjoys-in/BhaiyaChalo/services/ride-core/geofence-service/internal/constants"
...
func (p *kafkaPublisher) PublishGeofenceCreated(ctx context.Context, fence *model.Geofence) error {
	return p.publish(ctx, constants.EventGeofenceCreated, fence.CityID, fence)
}

func (p *kafkaPublisher) PublishGeofenceUpdated(ctx context.Context, fence *model.Geofence) error {
	return p.publish(ctx, constants.EventGeofenceUpdated, fence.CityID, fence)
}
