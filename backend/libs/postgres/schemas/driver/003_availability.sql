...
CREATE INDEX idx_driver_availability_comp   ON public.driver_availability (city_id, online, vehicle_type) WHERE online = TRUE;

-- Partitioned by timestamp for time-series retention
...
CREATE TABLE public.driver_availability_logs_default
    PARTITION OF public.driver_availability_logs DEFAULT;
...
CREATE INDEX idx_driver_availability_logs_comp   ON public.driver_availability_logs (driver_id, action, timestamp);
